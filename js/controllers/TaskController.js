...
class TaskController {
    ...
    try {
      ...
      if (!taskDescription) {
        this.view.showMessage(
          "Por favor, digite uma descrição para a tarefa",
          "error"
        );
        return false;
      }
  ...
  async editTask(taskId) {
    try {
      ...
      const newDescription = await showEditModal(task.description);
  ...
  async deleteTask(taskId) {
    try {
      ...
      const userConfirmed = await showConfirmModal(confirmMessage);
      if (!userConfirmed) {
        return false;
      }
